...
int main()
{
	...
	texture->LoadTexture("gezi.bmp");
	phongShader = new PhongShader(PhongVert(), PhongFrag(texture, depthBuffer));
...
// 	phongShader->frag.depthBuffer = depthBuffer;
	...
	planeShader = new PhongShader(PhongVert(), PhongFrag(texture, depthBuffer));;
...
void CreateSphere(float radius, int slices, int stacks)
{
	...
	{
		phi = PI * (i / (float)stacks);
		for (int j = 0; j < slices; ++j)
		{
			theta = 2 * PI * (j / (float)slices);
			...
			// Add vertex data to your mesh
			currentMesh->AddVertexData(x, y, z, s, t, Vector3f(x, y, z).Normalize(), Color(1, 1, 1, 1));
		}
	}
}
...
{
	float radius = 1.0f;
	int slices = 30; // Number of longitudinal subdivisions
	int stacks = 20; // Number of latitudinal subdivisions

	CreateSphere(radius, slices, stacks);

	// Apply transformation to the sphere object
	Matrix M;
	Matrix s = m->GetTransform().Scale(Vector3f(1, 1, 1)); // Scale
	Matrix r = m->GetTransform().Rotate(Vector3f(0, 0, 0)); // Rotation
	Matrix t = m->GetTransform().Translate(Vector3f(0, 0, 0)); // Translation
	M = t * r * s; // Note the order of matrix multiplication

	// Set transformation matrix to shader
	planeShader->v2f.m = M;

	// Set object-to-world transformation for the sphere
	currentMesh->SetObjectToWorld(M);
}
